...
function PrevBookings() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      setBookings(response.data.filter((item) => (item.email).toLowerCase() === user.email && item.status === 'confirmed'));
      setLoading(false);
    } catch (err) {
      setError(err);
      setLoading(false);
        ...
        <div className="row">
            <div className="col-md-4 m-auto">
                <div className="card w-100 p-3 text-center">
                    ...
                    <h1 className="fs-5 fw-bold">No Previous Bookings found</h1>
                </div>
            </div>
        </div>
    ):(
     <>
      <Typography variant="h4" mb={3}>Previous Bookings</Typography>
