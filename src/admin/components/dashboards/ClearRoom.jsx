...
const ClearRoom = () => {
  const [selectedSporti, setSelectedSporti] = useState('SPORTI-1');
  ...
  const handleConfirmBooking = async () => {
    ...
    setLoading(true);
    try {
      await axios.patch(`https://sporti-backend-live-3.onrender.com/api/sporti/service/${formData._id}/select-room/${selectedRoom.id}`, {
  ...
  if(window.confirm('DO You want to Block the room')){
    setLoading(true);
    try {
      await axios.post(`https://sporti-backend-live-3.onrender.com/api//clear/room/${id}`);
    ...
    toast.warning('Room Clear is cancelled')
  }
  };
                          ...
                          <Card.Body>
                            {roomData[floor][category].map((room) => (
                              <button
                                ...
                                onClick={room.isBooked?(() => blockRoom(room.id)):(()=>toast.info("This room is not booked"))}
                                // disabled={room.isBooked && !(selectedRoom && selectedRoom.id === room.id)}
                                style={{
                                  backgroundColor: room.isBooked ? "red" : "",
                                  borderColor: room.isBooked? "#007bff" : "",
                                  color: room.isBooked? "#fff" : "",
                                }}
                              >
                                {room.roomNumber}
                              </button>
